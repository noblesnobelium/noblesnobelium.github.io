...
(
	function ( $ ) {
		 ...
		 *
		 * @class MagazineX_Customizer_FrameWork
		 */
		var MagazineX_Customizer_FrameWork = {
			...
			checkControlVisibility : function ( control, id ) {
				...
				if ( ! _.isUndefined( control ) ) {
					...
					if ( 'undefined' != typeof MagazineXCustomizerControlsToggle[id] ) {
						var check            = false,
						    dependency_param = MagazineXCustomizerControlsToggle[id],
						...
						if ( 'undefined' !== typeof conditions ) {
							...
							if ( ! check ) {
								control.container.addClass( 'magazinex-hide' );
							} else {
								control.container.removeClass( 'magazinex-hide' );
							}
						}
					}
			...
			checkDependency : function ( conditions, values, compare_operator ) {
				...
				if ( _.isString( testValue ) ) {
					...
					if ( ! _.isUndefined( MagazineXCustomizerControlsToggle[testValue] ) ) {
						var conditions = ! _.isUndefined( MagazineXCustomizerControlsToggle[testValue]['conditions'] ) ? MagazineXCustomizerControlsToggle[testValue]['conditions'] : MagazineXCustomizerControlsToggle[testValue];
						var operator   = ! _.isUndefined( MagazineXCustomizerControlsToggle[testValue]['operator'] ) ? MagazineXCustomizerControlsToggle[testValue]['operator'] : 'AND';
						...
						if ( ! _.isUndefined( conditions ) ) {
							...
							if ( ! control.checkDependency( conditions, values, operator ) ) {
								...
								control_obj.container.removeClass( 'magazinex-hide' );
							}
						}
					}
					...
					$.each(
						...
						function ( index, val ) {
							...
							if ( 'undefined' !== typeof MagazineXCustomizerControlsToggle[cond_key] ) {
								var conditions = ! _.isUndefined( MagazineXCustomizerControlsToggle[cond_key]['conditions'] ) ? MagazineXCustomizerControlsToggle[cond_key]['conditions'] : MagazineXCustomizerControlsToggle[cond_key];
								var operator   = ! _.isUndefined( MagazineXCustomizerControlsToggle[cond_key]['operator'] ) ? MagazineXCustomizerControlsToggle[cond_key]['operator'] : 'AND';
			...
			hideEmptySections : function () {
				...
				$( 'ul.accordion-section.control-section-magazinex_section' ).each(
					function () {
						...
						if ( controls.length > 0 ) {
							controls.each(
								function () {
									if ( ! $( this ).hasClass( 'magazinex-hide' ) && $( this ).css( 'display' ) != 'none' ) {
										visibleIt = true;
									}
								}
							);
							...
							if ( ! visibleIt ) {
								$( '.control-section[aria-owns="' + parentId + '"]' ).addClass( 'magazinex-hide' );
							} else {
								$( '.control-section[aria-owns="' + parentId + '"]' ).removeClass( 'magazinex-hide' );
							}
						}
					}
				);
			...
			hasDependentControls : function ( control_id ) {
				...
				$.each(
					MagazineXCustomizerControlsToggle,
					function ( index, val ) {
		...
		$(
			function () {
				MagazineX_Customizer_FrameWork.init();
			}
		);
