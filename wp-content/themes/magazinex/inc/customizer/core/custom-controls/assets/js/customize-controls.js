...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['magazinex-background'] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				control.initMagazineXBackgroundControl();

			},

			initMagazineXBackgroundControl : function () {

				var control     = this,
				    value       = control.setting._value,
				    colorpicker = control.container.find( '.magazinex-color-picker-alpha' );
				...
				control.container.on( 'click', '.background-image-upload-remove-button', function ( e ) {
					...
					if ( preview.length ) {
						preview.removeClass().addClass( 'placeholder' ).html( MagazineXCustomizerControlBackground.placeholder );
					}
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor[ 'magazinex-color' ] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				var control = this,
					...
					picker = this.container.find( '.magazinex-color-picker-alpha' ),
 ...
 * @package MagazineX
 */
wp.customize.controlConstructor['magazinex-dimensions'] = wp.customize.Control.extend( {
 ...
 * @package MagazineX
 */
wp.customize.controlConstructor[ 'magazinex-editor' ] = wp.customize.Control.extend( {
 ...
 * @package MagazineX
 */
wp.customize.controlConstructor[ 'magazinex-dropdown-categories' ] = wp.customize.Control.extend( {
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['magazinex-fontawesome'] = wp.customize.Control.extend(
			{
				ready: function () {
					'use strict';
					...
					control.initMagazineXFontawesomeControl();
				},

				initMagazineXFontawesomeControl: function() {
					var control       = this,
						selector      = control.selector,
						elSelector    = $( selector ).find( 'select' ),
						faData        = [],
						value         = control.setting._value,
						data          = window['MagazineXCustomizerControlFontawesome' + this.id],
						faDataCounter = 0,
						faSelect;
...
(
	function ( $ ) {
		...
		wp.customize.controlConstructor['magazinex-group'] = wp.customize.Control.extend( {
			...
			ready : function () {
				...
				this.container.on( 'magazinex_settings_changed', control.onOptionChange );
			...
			registerToggleEvents : function () {
				...
				$( '.wp-full-overlay-sidebar-content' ).click(
					function ( e ) {
						if ( ! $( e.target ).closest( '.magazinex-field-settings-modal' ).length ) {
							$( '.magazinex-group-toggle-icon.open' ).trigger( 'click' );
						}
					}
				);
				...
				control.container.on(
					'click',
					'.magazinex-group-wrap .magazinex-group-toggle-icon',
					function ( e ) {
						...
						var $this          = $( this ),
						    parent_wrap    = $this.closest( '.customize-control-magazinex-group' ),
						    is_loaded      = parent_wrap.find( '.magazinex-field-settings-modal' ).data( 'loaded' ),
						    parent_section = parent_wrap.parents( '.control-section' );

						if ( $this.hasClass( 'open' ) ) {
							parent_wrap.find( '.magazinex-field-settings-modal' ).hide();
						} else {

							/* Close popup when another popup is clicked */
							var get_open_popup = parent_section.find( '.magazinex-group-toggle-icon.open' );
							if ( get_open_popup.length > 0 ) {
								get_open_popup.trigger( 'click' );
							}

							if ( is_loaded ) {
								parent_wrap.find( '.magazinex-field-settings-modal' ).show();
							} else {

								var fields     = control.params.magazinex_fields,
								    modal_wrap = $( MagazineXCustomizerControlGroup.group_modal_tmpl ),
								    device     = $( '#customize-footer-actions .active' ).attr( 'data-device' );

								parent_wrap.find( '.magazinex-field-settings-wrap' ).append( modal_wrap );
								parent_wrap.find( '.magazinex-fields-wrap' ).attr( 'data-control', control.params.name );
								control.magazinex_render_field( parent_wrap, fields, control );
								parent_wrap.find( '.magazinex-field-settings-modal' ).show();
				...
				control.container.on(
					'click',
					'.magazinex-group-wrap > .customizer-text',
					function ( e ) {

						e.preventDefault();
						e.stopPropagation();

						$( this ).find( '.magazinex-group-toggle-icon' ).trigger( 'click' );
			...
			magazinex_render_field : function ( wrap, fields, control_element ) {

				var control             = this;
				var magazinex_field_wrap = wrap.find( '.magazinex-fields-wrap' );
				var fields_html         = '';
				var control_types       = [];
				...
				if ( 'undefined' != typeof fields.tabs ) {
					...
					var counter = 0;
					fields_html += '<div id="' + control_element.params.name + '-tabs" class="magazinex-group-tabs">';

					fields_html += '<ul class="magazinex-group-list">';
					_.each(
						fields.tabs,
						function ( value, key ) {
							var li_class = '';
					...
					fields_html += '<div class="magazinex-tab-content" >';
					_.each(
						fields.tabs,
					...
					magazinex_field_wrap.html( fields_html );
				...
				_.each(
					...
					function ( control_type, index ) {
						...
						switch ( control_type.key ) {
							...
							case 'magazinex-color':
								control.initColorControl( magazinex_field_wrap, control_element, control_type.name );
								break;

							case 'magazinex-background':
								control.initBackgroundControl( control_element, control_type, control_type.name );
								break;

							case 'magazinex-typography':
				...
				wrap.find( '.magazinex-field-settings-modal' ).data( 'loaded', true );
				...
				_.each(
					...
					function ( attr, index ) {
						...
						var responsive_switchers = '',
						    controlsType         = [
							    'magazinex-typography'
						    ];

						if ( (
							     'magazinex-typography' === attr.control
						     ) && controlsType.includes( attr.control ) ) {
							attr.languages = MagazineXCustomizerControlTypographySubsets;
						}
			...
			initColorControl : function ( wrap, control_elem, name ) {
				...
				var control     = this;
				var colorpicker = wrap.find( '.customize-control-magazinex-color .magazinex-color-picker-alpha' );
				...
				colorpicker.wpColorPicker(
					{
						change : function ( event, ui ) {
							...
							if ( 'undefined' != typeof event.originalEvent || 'undefined' != typeof ui.color._alpha ) {
								...
								var picker  = $( '#' + name + '.customize-control-magazinex-color .magazinex-color-picker-alpha' );
			...
			getTypographyFonts : function () {
				...
				if ( ! _.isUndefined( MagazineXCustomizerControlTypography ) ) {
					return MagazineXCustomizerControlTypography;
				}
 ...
 * @package MagazineX
 */
wp.customize.controlConstructor['magazinex-slider'] = wp.customize.Control.extend( {
	...
	ready : function () {
		...
		this.container.find( 'input[type=range]' ).on( 'input change', function () {
			var value        = jQuery( this ).val(),
				input_number = jQuery( this ).closest( '.slider-wrapper' ).find( '.magazinex-range-value .value' );
		...
		this.container.find( '.magazinex-slider-reset' ).click( function () {
			var wrapper       = jQuery( this ).closest( '.slider-wrapper' ),
				input_range   = wrapper.find( 'input[type=range]' ),
				input_number  = wrapper.find( '.magazinex-range-value .value' ),
 ...
 * @package MagazineX
 */
wp.customize.controlConstructor['magazinex-sortable'] = wp.customize.Control.extend( {
 ...
 * @package MagazineX
 */
wp.customize.controlConstructor['magazinex-toggle'] = wp.customize.Control.extend( {
 ...
 * @package MagazineX
 */
wp.customize.controlConstructor['magazinex-typography'] = wp.customize.Control.extend( {
